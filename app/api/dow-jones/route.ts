...
export async function GET() {
  try {
    // Use ^DJI (with caret) - this is the correct Yahoo Finance symbol for Dow Jones index
    ...
    return NextResponse.json(
      { error: 'Failed to fetch Dow Jones data', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
