...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    await verifyAdmin()
    const result = await db.getNewsletterPost(params.id)
    return NextResponse.json({ ok: true, post: result.data, error: result.error })
  } catch (error: any) {
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await db.updateNewsletterPost(params.id, updates)
